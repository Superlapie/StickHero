...
class CanvasGameRenderer(private val inputMapper: AndroidTouchInputMapper) {
    private val paint = Paint(Paint.ANTI_ALIAS_FLAG)
    private val arenaRenderer = ArenaRenderer(paint)
    private val hudRenderer = HudRenderer(paint)
    private val skeletonRenderer = StickmanSkeletonRenderer(paint)
    private val motionTrailRenderer = MotionTrailRenderer(paint)
    private val impactEffectRenderer = ImpactEffectRenderer(paint)

    fun draw(canvas: Canvas, snapshot: RenderSnapshot) {
        ...
        snapshot.fighters.forEach { skeletonRenderer.draw(canvas, it, WORLD_HEIGHT) }
